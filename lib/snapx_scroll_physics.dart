...
class SnapScrollPhysics extends ScrollPhysics {
  ...
  double _getTargetPixels(ScrollPosition position, double velocity) {
    double snapHeight = this.snapHeight ?? position.viewportDimension;
    bool scrollUp = position.userScrollDirection == ScrollDirection.reverse;

    var distance = .0;
    // 当移动到snapHeight开始考虑滑动惯性了
    if (position.pixels > snapHeight) {
      var duration = _flingDuration(velocity);
    ...
    double targetPixel;
    bool isFiling = velocity.abs() > minFlingVelocity.abs();
    ...
    targetPixel =
        targetPixel.clamp(position.minScrollExtent, position.maxScrollExtent);
    return targetPixel;
  }
